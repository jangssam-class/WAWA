...
const imageInfo=webPath=>{
  ...
  try{const b=fs.readFileSync(fp);if(b.length>=24&&b.toString('ascii',1,4)==='PNG')return {width:b.readUInt32BE(16),height:b.readUInt32BE(20)};}catch(e){}
  return null;
};
...
for(const p of posts){const url=`${SITE}/posts/${p.slug}.html`, f=faq(p), image=p.image?`${SITE}${p.image.startsWith('/')?p.image:'/'+p.image}`:'', imgInfo=imageInfo(p.image);
const articleJson={"@context":"https://schema.org","@type":"Article",headline:p.title,description:p.excerpt,datePublished:p.date,dateModified:p.date,author:{"@type":"Organization",name:"WAWA 소수정예학원"},publisher:{"@type":"Organization",name:"WAWA 소수정예학원"},mainEntityOfPage:url}; if(image)articleJson.image=imgInfo?{"@type":"ImageObject",url:image,width:imgInfo.width,height:imgInfo.height}:image;
